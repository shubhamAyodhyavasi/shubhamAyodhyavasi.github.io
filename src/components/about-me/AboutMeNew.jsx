    ...
    <section id="about" className="py-24 bg-white dark:bg-slate-900 transition-colors duration-300">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        >
          ...
          <h2 className="mt-2 text-3xl sm:text-4xl font-extrabold text-slate-900 dark:text-white">About Me</h2>
        </motion.div>
        ...
        >
          ...
          <motion.div variants={itemVariants} className="lg:col-span-2 flex flex-col gap-6">
            <div className="relative p-6 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 text-center">
              <div
              ...
              <h3 className="text-slate-900 dark:text-white font-bold text-lg">Shubham Ayodhyavasi</h3>
            ...
            <div className="p-6 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 space-y-3">
              {ABOUT_QUICK_INFO.map((item) => (
                <div key={item.label} className="flex items-center gap-3 text-sm">
                  <span className="text-base">{item.icon}</span>
                  <span className="text-slate-500 dark:text-slate-400 w-20 shrink-0">{item.label}</span>
            ...
            >
              {SKILLS.map((skill) => (
                <motion.div
                  key={skill.category}
                  variants={itemVariants}
                  className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 hover:border-indigo-500/40 transition-colors"
                >
                  <h4 className="text-slate-700 dark:text-slate-300 font-medium text-sm mb-2">{skill.category}</h4>
